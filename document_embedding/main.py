...
def cnn_branch(n_filters,k_size,d_rate,my_input):
    return Dropout(d_rate)(GlobalMaxPooling1D()(Conv1D(filters=n_filters,kernel_size=k_size,activation='relu')(my_input)))
...
doc_wv = Embedding(input_dim = len(word_to_index.keys()),output_dim = d,input_length=max_size)(doc_ints)### fill the gap (add an Embedding layer) https://keras.io/layers/embeddings/ ###
...
saliency_input = model.layers[3].input ### fill the gap (extract the rows of the embedding matrix from the model) ###
saliency_output = model.layers[10].output### fill the gap (get the probability distribution over classes from the model) ###
